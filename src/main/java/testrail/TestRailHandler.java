...
public class TestRailHandler {
	 ...
	 */
	private QAAnnotations.TestClassInfo getTestClassAnnotation(String testClass) throws Exception{
		//check if test class annotation exists in the class
		if (TestRailHandler.class.getClassLoader().loadClass((testClass)).isAnnotationPresent(QAAnnotations.TestClassInfo.class)){
			QAAnnotations.TestClassInfo testClassAnno = TestRailHandler.class.getClassLoader().loadClass((testClass)).getAnnotation(QAAnnotations.TestClassInfo.class);
	...
	public void pushTestInfoToTestRailByTestClass(String testClass) throws Exception {
		...
		try{
			...
			QAAnnotations.TestClassInfo testClassAnno = getTestClassAnnotation(testClass);
			...
			if (testClassAnno != null) {
				...
				for (Method method : TestRailHandler.class.getClassLoader().loadClass((testClass)).getMethods()) {
					...
					if (method.isAnnotationPresent(QAAnnotations.TestCaseInfo.class)) {
						try {
							//TODO:  check if test case is new or existing
							//TODO:  if new
								// iterates through all the annotations available in the method
								Map<String, String> data = new HashMap<String, String>();
								for (QAAnnotations.TestCaseInfo anno : method.getDeclaredAnnotationsByType(QAAnnotations.TestCaseInfo.class)) {
	...
	public void updateResultToTestRail(int value, String case_Id, String runid, String comment) throws Exception {
